...
class SoalEmpatBelasActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_soal_empat_belas)
        ...
        var status =0
        if (bundles?.getString("s14").toString()=="a"){
            pilihanA.isChecked=true
        } else if (bundles?.getString("s14").toString()=="b") {
            pilihanB.isChecked=true
            status=1
        } else if (bundles?.getString("s14").toString()=="c") {
            pilihanC.isChecked=true
        } else if (bundles?.getString("s14").toString()=="d") {
        ...
        btn_quiz_forward.setOnClickListener {
            var jawaban = '0'
            val intent = Intent(this, SoalLimaBelasActivity::class.java)
            ...
            bundle.putString("s14",jawaban.toString())
        ...
        btn_quiz_back.setOnClickListener {
            var jawaban = '0'
            val intent = Intent(this, SoalTigaBelasActivity::class.java)
            if (pilihanA.isChecked){
                skor = skor - status
                jawaban = 'a'
            } else if (pilihanB.isChecked) {
