...
import kotlinx.android.synthetic.main.activity_soal_lima_belas.*
import kotlinx.android.synthetic.main.activity_soal_satu.*
import kotlinx.android.synthetic.main.activity_soal_satu.btn_quiz_back

class SoalLimaBelasActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_soal_lima_belas)
        ...
        var status =0
        if (bundles?.getString("s15").toString()=="a"){
            pilihanA.isChecked=true
            status=1
        } else if (bundles?.getString("s15").toString()=="b") {
            pilihanB.isChecked=true
        } else if (bundles?.getString("s15").toString()=="c") {
            pilihanC.isChecked=true
        } else if (bundles?.getString("s15").toString()=="d") {
            pilihanD.isChecked = true
        }

        var skor = bundles?.getString("ts").toString().toInt()
        btn_quiz_submitf.setOnClickListener {
            var jawaban = '0'
            val intent = Intent(this, QuizHasilActivity::class.java)
            ...
            bundle.putString("s8",bundles?.getString("s8").toString())
            ...
            bundle.putString("s15",jawaban.toString())
        ...
        btn_quiz_back.setOnClickListener {
            var jawaban = '0'
            val intent = Intent(this, SoalEmpatBelasActivity::class.java)
            if (pilihanA.isChecked){
                skor = skor - status +1
                jawaban = 'a'
            } else if (pilihanB.isChecked) {
